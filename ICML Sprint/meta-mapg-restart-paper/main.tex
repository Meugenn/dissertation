...
\begin{figure}[t]
  ...
  \caption{Restart budget as equilibrium-selection budget in Stag Hunt. With paired restart initialisations, we plot the probability of discovering the payoff-dominant equilibrium by budget $K$; bands show 95\% confidence intervals. The inset shows $\sum_{k\le K}(8-W_k)$, the cumulative gap below best welfare. Lower is better. Meta-MAPG reaches the good basin with fewer restarts.}
  \label{fig:restart}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{Two-phase schedule vs constant shaping in Stag Hunt ($80$ seeds, $260$ steps, $\lambda_c=1.5$). PG reaches $31\%$, constant-shaping Meta-MAPG reaches $41\%$, and the two-phase schedule reaches $43\%$. Annealing restores Nash consistency without losing basin-entry performance. Right: $\lambda_n$ over time; the dotted line marks $N_0=100$.}
  \label{fig:annealing}
\end{figure}
