...
This paper does not claim unconditional convergence of arbitrary opponent-aware learning in deep MARL. It shows that finite-unroll Meta-MAPG can be analysed within classical stochastic approximation under boundedness, smoothness, Nash-consistency, and summability conditions, and that local convergence can be globalised by independent restarts when local basin mass is positive. The MLP-policy check in \cref{app:mlp} only verifies that the qualitative peer-vs-nonpeer separation survives a modest neural parameterisation.
...
We spell out the stochastic-approximation step used in \cref{lem:local}. The point is not to reprove \citet{giannou2022convergence}, but to show explicitly that the Meta-MAPG recursion has the same Lyapunov form after the estimator decomposition.
...
The experiment scripts, generated figures, raw CSVs, and run configurations are included with the submission. The tabular script saves its command-line configuration and all CSV outputs under \texttt{artifacts/main}. The raw outputs cover the ablation, restart, trajectory, basin, peer-sweep, annealing, and estimator-sanity figures. The code uses no exact gradients for updates; expected returns are computed only for logging and evaluation.
...
To check that the ablation is not purely a tabular artefact, we repeat IPD with a two-layer MLP policy for each agent: a 5-dimensional one-hot previous-action input, two tanh hidden layers of width 16, and one Bernoulli cooperation logit. This scales the policy from 5 tabular logits to about 385 parameters per agent. We keep the same 260-step budget, batch size, seed schedule, peer coefficient, and own coefficient as the tabular experiment. Higher-order terms are computed by PyTorch autograd using DiCE-style surrogate losses \citep{foerster2018dice}.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.82\linewidth]{figures/mlp_ipd.pdf}
  \caption{Non-tabular IPD check with MLP policies. Cooperative success rate on IPD with two-layer MLP policies, 100 seeds, and Wilson 95\% confidence intervals. Peer-aware methods remain above non-peer methods under the same shaping coefficients used in the tabular experiments: PG reaches $0\%$, Meta-PG $2\%$, peer-only $11\%$, and Meta-MAPG $23\%$. This is a proof-of-transport for the mechanism, not a deep-MARL benchmark.}
  \label{fig:mlp-ipd}
\end{figure}
