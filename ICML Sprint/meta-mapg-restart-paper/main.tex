...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/restart_efficiency.pdf}
  \caption{Restart efficiency. With the same restart budget and threshold, Meta-MAPG requires fewer restarts than PG. In IPD, Meta-MAPG succeeds in $85\%$ of runs with mean $5.30$ restarts, while PG succeeds in $30\%$ with mean $10.70$ restarts. In Stag Hunt, both succeed in all runs, but Meta-MAPG reduces mean restarts from $1.65$ to $1.00$.}
  \label{fig:restart}
\end{figure}
...
The empirical claims are modest. The ablation primarily shows that peer-learning terms matter; it does not prove full Meta-MAPG dominates every LOLA-style variant in every game. The restart and basin experiments are the direct support for \cref{thm:restart,prop:basin}: opponent-aware shaping increases the measured basin in Stag Hunt and reduces the observed restart count under the same reinitialisation procedure. The estimator sanity check supports \cref{lem:decomp} by verifying that the implemented sample estimator has controlled variance as batch size increases.

\section{Discussion}

The result is local before it is global. Meta-MAPG inherits Giannou et al.'s stable-Nash theory only inside attracting neighbourhoods and only after the estimator has been placed in the stochastic-approximation form. Restarts remove dependence on a particular initialisation, but they do not identify the best equilibrium without an additional selection rule. Their expected complexity depends on the basin probability $p_\B$; opponent-aware shaping helps only when it increases this probability.
...
The raw files are \texttt{ablation\_summary.csv}, \texttt{restart\_summary.csv}, \texttt{basin\_map.csv}, and \texttt{estimator\_sanity.csv} under \texttt{artifacts/main}. The code uses no exact gradients for updates; expected returns are computed only for logging and evaluation.
